...
class ImageParser:
    ...
    def optimize_image(self, img):
        ...
        # Modify the mask: Set foreground areas to gray (128)
        mask2 = np.where((mask == 0) | (mask == 2), 128, 0).astype('uint8')

        # Create an output image where foreground pixels are gray and background is preserved
        foreground = np.full_like(img, (10, 10, 10), dtype=np.uint8)
        combined_image = np.where(mask2[:, :, np.newaxis] == 128, foreground, img)

        # Convert to YUV for contrast enhancement
        img_yuv = cv2.cvtColor(combined_image, cv2.COLOR_BGR2YUV)
